...
export const Route = createFileRoute('/_private/projects/$projectId')({
  ...
  loader: async ({ params }) => {
    ...
    const projects = getFromStorage(`${PROJECT_UNITS}/${projectId}`, {} as TProjectsTemp)
...
function RouteComponent() {
  ...
  const params: { projectId: string; unitId: string; moduleId: string } = Route.useParams()
  ...
  const handleAddNewUnit = (data: AddUnitFormSchema) => {
    ...
    setTabs((prev) => {
      ...
      setToStorage(`${PROJECT_UNITS}/${project.uuid}`, {
        [project.uuid]: newTabs
      })
      return newTabs
    })
  }
  ...
  useEffect(() => {
    if (tabs.length > 0 && !params.unitId) {
      history.pushState({}, '', `/projects/${project.uuid}/${tabs[0].id}`)
      ...
      if (unit) {
        ...
        history.pushState({}, '', `/projects/${project.uuid}`)
        setSelectedTab('')
      }
    } else {
      setSelectedTab('')
