  ...
  const componentTrigger =
    viewMode === 'table' ? (
      ...
      <div className='flex h-60 w-full flex-col items-center justify-center overflow-hidden rounded-lg bg-white p-4 shadow-md shadow-zinc-600 transition-all duration-500 hover:cursor-pointer hover:shadow-xl md:w-1/3 lg:w-1/4 xl:max-w-100 dark:bg-zinc-800 dark:shadow-zinc-900'>
        ...
        <span className='text-lg font-medium'>{t('projectsPage.addProject')}</span>
      </div>
    )
  ...
  return (
    <div>
      <div className='mb-2 flex justify-end gap-1'>
        {viewMode === 'table' && <DrawerAddProject componentTrigger={componentTrigger} />}
        <Button variant={viewMode !== 'table' ? 'default' : 'outline'} onClick={toggleViewMode}>
          <LayoutGrid className='h-4 w-4' />
        </Button>
        <Button variant={viewMode !== 'grid' ? 'default' : 'outline'} onClick={toggleViewMode}>
          <List className='h-4 w-4' />
        </Button>
      </div>
      {viewMode === 'table' ? (
        <ProjectTable projects={data?.data.projects ?? []} onClickProject={onClickProject} />
      ) : (
