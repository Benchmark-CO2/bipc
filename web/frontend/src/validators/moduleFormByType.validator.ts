...
const steelMaterialItemSchema = z
  .object({})
  .catchall(z.unknown())
  .optional()
  .nullable();
...
export function createModuleV2FormSchema() {
  const beamColumnDataSchema = z
    .object({
      floor_ids: z.array(z.string()).optional().nullable(),
      floor_index: z
        .union([permissiveNumber, z.array(z.number().int().nonnegative())])
        .optional(),
  ...
  const concreteWallDataSchema = z
    .object({
      floor_ids: z.array(z.string()).optional().nullable(),
      floor_index: z
        .union([permissiveNumber, z.array(z.number().int().nonnegative())])
  ...
  const structuralMasonryDataSchema = z
    .object({
      floor_ids: z.array(z.string()).optional().nullable(),
      floor_index: z
        .union([permissiveNumber, z.array(z.number().int().nonnegative())])
  ...
  const raftFoundationDataSchema = z
    .object({
  ...
  const pilesFoundationDataSchema = z
    .object({
      unit_id: permissiveString,
      concrete: z.array(z.unknown()).optional().nullable(),
      steel: z.array(z.unknown()).optional().nullable(),
  ...
  const raftPilesFoundationDataSchema = z
    .object({
      unit_id: permissiveString,
      raft_area: permissiveNumber,
      raft_thickness: permissiveNumber,
