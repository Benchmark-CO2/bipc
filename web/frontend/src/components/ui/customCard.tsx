...
import DrawerEditProject from '../layout/drawer-edit-project';
...
const CustomCard = ({ onClick, project }: CustomCardProps) => {
  ...
  const { background_image, city, description, name, state, users_len: users } = project
  ...
  return (
    ...
    >
      <div className='group relative h-60 w-full max-sm:h-40'>
          <ModalConfirmDelete key={project.uuid} projectUUID={project.uuid} />
          <DrawerEditProject componentTrigger={<Edit size={20} color='#FFF' className='delete-project z-50 absolute right-10 top-2 hover:shadow-md'  />} projectData={project} />
