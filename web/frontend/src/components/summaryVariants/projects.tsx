...
const colors = ['bg-pink-500', 'bg-yellow-500', 'bg-green-500', 'bg-blue-500', 'bg-purple-500', 'bg-red-500']
...
const ProjectsSummary = ({ projects, data }: ProjectsSummaryProps) => {
  ...
  const managedData = manageData(data.benchmark?.[type as "co2" | "energy"] || []).map(el => ({
        ...el,
  ...
  const handleAddProject = (projectId: string) => {
    ...
    useEffect(() => {
      setSelectedProjects(selectedProjects.filter(id => projects.find(u => u.id === id) !== undefined))
    }, [projects])
      ...
      <div className="flex flex-col items-start w-full">
        ...
        <ul className="flex flex-col gap-2 text-xl w-full text-black">
          {(!stackedData || stackedData.length === 0) && (
            <li className="text-gray-500">Nenhum projeto selecionado.</li>
          )}
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 cursor-pointer">
                  <Checkbox checked={selectedProjects.includes(project.id)} onClick={() => handleAddProject(project.id)} />
                  <h4 className="whitespace-nowrap flex items-center gap-3 cursor-pointer">{project.label}</h4>
                </div>
