...
const permissionsOptions: Array<{
  value: string;
  label: "drawerInvite.viewPermission" | "drawerInvite.editPermission";
}> = [
...
const DrawerInvite = ({ projectId }: { projectId: string }) => {
  ...
  const { mutate } = useMutation({
    mutationFn: ({
      ...
      postSendInvite(projectId, email, permissions),
    onSuccess: () => {
                  ...
                  <FormControl className="w-full">
                    ...
                    >
                      <SelectTrigger className="w-full">
                        <SelectValue
                          placeholder={t("drawerInvite.projectsPlaceholder")}
                        />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {permissionsOptions.map((permission, idx, self) => (
                    <FormItem
                          ...
                          onChange={(e) => {
                            ...
                            if (e.target.checked) {
                              field.onChange([
                                ...new Set([
                                  ...field.value,
                                  ...self.slice(0, idx + 1).map((p) => p.value),
                                ]),
                              ]);
                            } else {
                              field.onChange(
