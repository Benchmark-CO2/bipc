...
func (m PermissionModel) GetAllForUser(userID uuid.UUID, projectID uuid.UUID) (Permissions, error) {
	query := `
        SELECT p.code
        FROM permissions p
...
func (m PermissionModel) SetForUser(userID uuid.UUID, projectID uuid.UUID, codes ...string) error {
